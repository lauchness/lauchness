...
- The **View** is the JSX which relies on the `fullTime` value to render, and can call the `setFullTime` function from a user interaction
...
- The **View** is the JSX which relies on the `state` value to render, and can call the `dispatch` function from a user interaction
...
To make sure we don't introduce typos into our code, it's common to define the action types as constants and make **action creators** for each one. An action creator is a function which returns one of our action object. For our `SET_FULL_TIME` action, we could do the following.
...
So now we can see the full picture. The useReducer takes a **reducer function** and an **initial state**, it then returns a **state** and a function to **dispatch** actions. Any action objects that get dispatched will run through the reducer function to return a new state.
...
**Next, read about the useReducer as a [state machine](/blog/useReducer-state-machine)**
