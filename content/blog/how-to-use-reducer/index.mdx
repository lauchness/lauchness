...
It's common for the reducer to use a [switch statement](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/switch) and define a case for every action type we want to use. We should always define a `default` case, where we simply return the current state, that way if our reducer's switch statement doesn't have a case for the action passed in, it won't return `undefined` (this really shouldn't happen, but better safe than sorry). If do this with the reducer from our example above, it could look like this.
